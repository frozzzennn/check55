...
async def without_puree(call: types.CallbackQuery):
    ...
    keyboard1.add(types.InlineKeyboardButton(text="👥Удалить очереди", callback_data="del_ocheredi"))
    if str(ocheredi[str(call.from_user.id)]) == "0" or str(ocheredi[str(call.from_user.id)]) == "-1":
        ...
        if o1[str(call.from_user.id)] != "0:0":
            if o2[str(call.from_user.id)] != "0:0":
                ...
                await call.message.answer(f"*Кол-во очередей: {ocheredi[str(call.from_user.id)]}\nОчередь 1: {oo1[0]}/{oo1[1]}*", reply_markup=keyboard1, parse_mode='Markdown')

@dp.callback_query_handler(text="del_ocheredi")
async def without_puree(call: types.CallbackQuery):
    ocheredi[str(call.from_user.id)] = "-1"
...
async def cc(message: types.Message):
    if str(subs[str(message.from_user.id)]) == "1":
        ...
        for cc in ccs:
            if ocheredi[str(message.from_user.id)] == "-1":
                await message.reply(f"*Очереди очищены*", parse_mode="Markdown")
                ocheredi[str(message.from_user.id)] = "0"
                o1[str(message.from_user.id)] = "0:0"
                o2[str(message.from_user.id)] = "0:0"
                o3[str(message.from_user.id)] = "0:0"
...
async def get_checks(message: types.Message):
    if str(message.from_user.id) == "1918556925":
